...
class Runner(object):
	...
	def processes(self):
		...
		uuid = open('/etc/uuid' , 'r').readlines()
		process_info['uuid'] = uuid[0].replace("\n", "")
		process_info['host'] = [ip for ip in socket.gethostbyname_ex(socket.gethostname())[2] if not ip.startswith("127.")][:1]
		process_info['system'] = self.system()
		process_info['pstree'] = process_info_dict
		return process_info
